    ...
    def _ol_one_batch(self,dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        for _ in range(self.n_inner):
            ...
            if not self.buffer.is_empty():
                ...
                # decoder input
                if self.args.padding==0:
                    buff_dec_inp = torch.zeros([buff_y.shape[0], self.args.pred_len, buff_y.shape[-1]]).float().to(self.device)
                elif self.args.padding==1:
                    buff_dec_inp = torch.ones([buff_y.shape[0], self.args.pred_len, buff_y.shape[-1]]).float().to(self.device)
                ...
                loss += 0.2 * criterion(out, buff_true)
            loss.backward()
            self.opt.step()       
