...
class Exp_TS2VecSupervised(Exp_Basic):
    def __init__(self, args):
        ...
        self.n_replay = args.n_replay    ###    
        self.er_loss_weight = args.er_loss_weight   ###
        try:    ###
            self.test_lr = args.test_lr     ###
        except:   ###
            self.test_lr = None     ###
    ...
    def test(self, setting):
        ...
        # set learning rate to test_lr
        if self.test_lr:
            for param_group in self.opt.param_groups:
                param_group['lr'] = self.test_lr
        ######
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):    #tqdm(test_loader)):
            pred, true = self._process_one_batch(
    ...
    def _ol_one_batch(self,dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        for _ in range(self.n_inner):
            ...
            if not self.buffer.is_empty():
                buff_x, buff_y, idx = self.buffer.get_data(self.n_replay)
                out = self.model(buff_x)
                loss += self.er_loss_weight * criterion(out, buff_y)
            loss.backward()
            self.opt.step()       
