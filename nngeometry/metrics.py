    ...
    **kwargs,
    ...
    if variant == "classif_logits":
        ...
        def fim_function(*d):
            ...
            dist = StudentT(
                df=deg_f, loc=torch.zeros_like(estimates), scale=scale
            )
            samples = dist.sample((trials,))
            
            denom   = (deg_f * scale**2) + samples**2
            w       = ((deg_f + 1) * samples) / denom
            lambda_ = torch.ones_like(estimates) * lambd
            lambda_[new_idxs] = 1
            w *= (lambda_ ** 0.5).unsqueeze(0)
            return trials**-0.5 * (w * estimates).sum(dim=-1)
    ...
    if 'n_output' in kwargs:
        generator = TorchHooksJacobianBackend(
        model=model, function=fim_function, verbose=verbose, n_output=kwargs['n_output']
        )
    else:
        generator = TorchHooksJacobianBackend(
...
def FIM(
    ...
    **kwargs,
    ...
    function_fim = partial(SQRT_VAR[variant], function, **kwargs)
...
def _sqrt_var_regression(function, *d, **kwargs):
    estimates = function(*d)
    return estimates

def _sqrt_var_student_t(function, *d, **kwargs):
    estimates = function(*d)
...
SQRT_VAR = {
    ...
    FIM_Types.STUDENT: _sqrt_var_student_t,
}
